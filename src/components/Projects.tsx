  ...
  return <section id="projects" className="section-spacing bg-secondary/30">
      <div className="container-main">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-20">
          {houses.map((house, index) => <div key={index} className="bg-card rounded-xl overflow-hidden border border-border hover:shadow-lg transition-all duration-300">
              <div className="aspect-[4/3] overflow-hidden">
                <img src={house.image} alt={`Дом ${house.area}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
              </div>
              ...
              <div className="p-5">
                ...
                {house.landIncluded && <div className="inline-block text-xs bg-accent/10 text-accent px-2 py-1 rounded mb-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs text-foreground mb-4">
                  {house.specs.map((spec, specIndex) => <div key={specIndex} className="flex items-center gap-1 bg-secondary/50 px-2 py-1.5 rounded">
        ...
        <div>
          ...
          <div className="space-y-8">
            {villages.map((village, index) => <div key={index} className="bg-card rounded-xl overflow-hidden border border-border hover:shadow-lg transition-all duration-300">
                <div className="grid md:grid-cols-2 gap-0">
                  ...
                  <div className="p-5 md:p-6 lg:p-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-3 mb-4 md:mb-6">
                      {village.features.map((feature, fIndex) => <div key={fIndex} className="flex items-center gap-2 text-xs md:text-sm">
                          <div className="w-1.5 h-1.5 rounded-full bg-accent shrink-0" />
                          <span className="text-foreground">{feature}</span>
                        </div>)}
                    </div>
                    <a href={village.websiteUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-accent hover:text-accent/80 transition-colors font-medium text-sm md:text-base">
                      <ExternalLink className="w-4 h-4" />
                      Официальный сайт
                    </a>
                  </div>
